...
//スワップチェーンからバックバッファを取得（バックバッファ ＝ レンダーターゲット）
	...
	hr = pDevice_->CreateBlendState(&BlendState, &pBlendState_);
	if (FAILED(hr)) {
...
HRESULT Direct3D::InitToonShader()
{
	...
	// 頂点シェーダの作成（コンパイル）
	ID3DBlob* pCompileVS = nullptr;
	hr = D3DCompileFromFile(L"ToonShader.hlsl", nullptr, nullptr, "VS", "vs_5_0", NULL, 0, &pCompileVS, NULL);
	assert(pCompileVS != nullptr);
	...
	SAFE_RELEASE(pCompileVS)
		// ピクセルシェーダの作成（コンパイル）
		ID3DBlob* pCompilePS = nullptr;
	hr = D3DCompileFromFile(L"ToonShader.hlsl", nullptr, nullptr, "PS", "ps_5_0", NULL, 0, &pCompilePS, NULL);
	if (FAILED(hr)) return hr;
	assert(pCompilePS != nullptr);
...
HRESULT Direct3D::InitOutLineShader()
{
	...
	// 頂点シェーダの作成（コンパイル）
	ID3DBlob* pCompileVS = nullptr;
	hr = D3DCompileFromFile(L"outline.hlsl", nullptr, nullptr, "VS", "vs_5_0", NULL, 0, &pCompileVS, NULL);
	assert(pCompileVS != nullptr);
	...
	SAFE_RELEASE(pCompileVS)
		// ピクセルシェーダの作成（コンパイル）
		ID3DBlob* pCompilePS = nullptr;
	hr = D3DCompileFromFile(L"outline.hlsl", nullptr, nullptr, "PS", "ps_5_0", NULL, 0, &pCompilePS, NULL);
	if (FAILED(hr)) return hr;
	assert(pCompilePS != nullptr);
